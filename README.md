...
site/
  ...
  lore.html         ← 세계관 페이지. ★손으로 쓴 것 (빌드 대상이 아니다)
  css/design.css    ← 클로드 디자인 시안이 만든 규칙. 역시 빌드 산출물
  css/extra.css     ← 손으로 쓴 것. 서체·영상·BT 뷰어·좁은 화면 보정 + 세계관 낱말
  css/lore.css      ← 손으로 쓴 것. 세계관 페이지 전용
  js/main.js        ← 손으로 쓴 것. 영상 지연 로딩 · 목차 · BT 뷰어 · 클릭 로딩
  js/lore.js        ← 손으로 쓴 것. 세계관 용어 사전 (데이터 + 검색 + 딥링크)
...
기본은 `index.html`(포폴)이고 `lore.html`(세계관)이 따로 있다.

- **포폴 → 세계관**: 본문의 낱말에 점선 밑줄이 그어져 있고, 누르면 **새 탭**으로
  `lore.html#낱말` 이 열린다. 어떤 낱말을 걸지는 `tools/build_site.py` 의 `LORE_TERMS` 에 있다.
  ★**index.html 을 손으로 고치지 말고 그 목록을 고친 뒤 다시 굽는다.**
- **세계관 → 포폴**: 상단·하단에 「포트폴리오」 링크.
- 용어 데이터는 `js/lore.js` 의 `G` 배열 하나에 있다. `t: 'ours'` 는 우리가 만든 것,
  `t: 'src'` 는 원작에서 온 것이다. 항목 id 는 **낱말에서 공백만 하이픈**으로 바꾼 것이다.

⚠ `tools/build_artifact.py` 는 아직 `index.html` 한 장만 묶는다.
**Artifact 한 파일 판에는 세계관 페이지가 안 들어간다.** GitHub Pages 쪽은 문제없다.
